...
export function createStaffConfig(roleOptions: SelectOption[]): ManagementConfig {
  return {
    ...
    fields: [
      ...
      { name: 'password', label: '登录密码', type: 'text', required: true },
...
export const companiesConfig: ManagementConfig = {
  entity: 'companies',
  title: '分公司管理',
  description: '管理分公司基础资料、积分额度、门店数量和当前经营状态。',
  ...
  columns: [
    ...
    { key: 'available_points', title: '可用积分', type: 'currency' },
    { key: 'total_points', title: '累计积分', type: 'currency' },
    { key: 'status', title: '状态', type: 'badge' }
  ],
  fields: [
    ...
    { name: 'available_points', label: '可用积分', type: 'number', required: true },
    { name: 'total_points', label: '累计积分', type: 'number', required: true },
...
export function createPurchaseOrdersConfig(
  ...
  return {
    ...
    title: '订货单管理',
    description: '处理总部供货到分公司的积分订货、审核和入库流转。',
    ...
    columns: [
      ...
      { key: 'points_total', title: '积分消耗', type: 'currency' },
      { key: 'abnormal_flag', title: '异常标记', type: 'badge' },
      { key: 'created_at', title: '创建时间' }
    ],
    fields: [
      { name: 'order_no', label: '订货单号', type: 'text', required: true },
      { name: 'company_id', label: '分公司', type: 'select', required: true, options: companyOptions },
      { name: 'sku_id', label: 'SKU', type: 'select', required: true, options: skuOptions },
      { name: 'quantity', label: '订货数量', type: 'number', required: true },
      {
        name: 'status',
        label: '初始状态',
        type: 'select',
...
export function createMemberOrdersConfig(
  ...
  return {
    ...
    title: '会员订单管理',
    description: '承接门店面向会员的销售结果、销售归属和核销流水。',
    viewPermission: 'member-orders:view',
    writePermission: 'member-orders:edit',
    searchPlaceholder: '搜索会员订单号、会员、分公司、门店或销售员工',
    filterLabel: '会员订单状态',
    filterKey: 'status',
    filterOptions: [
      { label: '全部状态', value: 'all' },
    ...
    columns: [
      { key: 'order_no', title: '会员订单号', type: 'code' },
      { key: 'company_name', title: '分公司' },
      { key: 'store_name', title: '门店' },
      { key: 'member_name', title: '会员' },
      { key: 'sales_staff_name', title: '销售归属' },
      { key: 'total_amount', title: '订单金额', type: 'currency' },
      { key: 'status', title: '状态', type: 'badge' }
    ],
    fields: [
      { name: 'order_no', label: '会员订单号', type: 'text', required: true },
      { name: 'company_id', label: '分公司', type: 'select', required: true, options: companyOptions },
      ...
      { name: 'member_name', label: '会员姓名', type: 'text', required: true },
      { name: 'member_phone', label: '会员手机号', type: 'text', required: true },
